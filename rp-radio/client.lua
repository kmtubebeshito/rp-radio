...
Radio.Labels = {        
	{ "FRZL_RADIO_HELP", "~s~" .. (radioConfig.Controls.Secondary.Enabled and "~" .. radioConfig.Controls.Secondary.Name .. "." .. radioConfig.Controls.Activator.Name .. "~" or "~" .. radioConfig.Controls.Activator.Name .. "~") .. " pulsa F11 para ocultar.~n~~" .. radioConfig.Controls.Toggle.Name .. "~ para poner la radio ~g~on~s~.~n~~" .. radioConfig.Controls.Decrease.Name .. "~ y ~" .. radioConfig.Controls.Increase.Name .. "~ para cambiar el canal~n~~" .. radioConfig.Controls.Input.Name .. "~ para seleccionar canal~n~~" .. radioConfig.Controls.ToggleClicks.Name .. "~ para ~a~ clics del micro~n~Canal: ~1~ MHz" },
	{ "FRZL_RADIO_HELP2", "~s~" .. (radioConfig.Controls.Secondary.Enabled and "~" .. radioConfig.Controls.Secondary.Name .. "." .. radioConfig.Controls.Activator.Name .. "~" or "~" .. radioConfig.Controls.Activator.Name .. "~") .. " pulsa F11 para ocultar.~n~~" .. radioConfig.Controls.Toggle.Name .. "~ para poner la radio ~r~off~s~.~n~~" .. "~ Manten ALT para hablar.~n~ Canal: ~1~ MHz" },
...
Radio.Commands = {
	...
	},
	{
		...
		Params = {
			{name = "number", "Introduce el numero de canal"}
		},
		Handler = function(src, args, raw)
			if Radio.Has then
				if args[1] then
...
function newRadioProp()
	...
	local bone = GetPedBoneIndex(myPedId, 60309)
	AttachEntityToEntity(radioProp, myPedId, bone, 0.06, 0.05, 0.04, 90.0, 520.0, 0.0, 1, 1, 0, 0, 2, 1)
end
		...
		if Radio.Open then
			...
			if radioConfig.AllowRadioWhenClosed then
				if Radio.Has and Radio.On and isBroadcasting and not isPlayingBroadcastAnim then
					...
					-- prop y anim para spain academy ||| Kiwisita
					myPedId = PlayerPedId()
					newRadioProp()
				...
				elseif not isBroadcasting and isPlayingBroadcastAnim then
					...
					Citizen.Wait(0)
					-- prop y anim para spain academy ||| Kiwisita
					nopuedoapuntar = false
					NetworkRequestControlOfEntity(Handle2)
					StopAnimTask(playerPed, dictionary, animation, 1.0)
					StopAnimTask(playerPed, broadcastDictionary, broadcastAnimation, -4.0)
					deleteRadio()
				end
